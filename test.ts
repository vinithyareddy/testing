onLoadColumnChart() {
  ...
  this.chartOptions = {
    ...
    // ✅ Grouped X-axis - tight spacing between Planned/Actuals
    xAxis: {
      categories: [
    ...
    plotOptions: {
      column: {
        stacking: 'normal',
        borderWidth: 0,
        groupPadding: 0.05, // ⬅️ tighter space between Planned/Actuals
        pointPadding: 0.05,
        pointWidth: 25,
        dataLabels: {
          enabled: true,
          formatter: function () {
            // ⬅️ Make middle bar text white (Build)
    ...
    // ✅ Series (ordered for stack layout)
    series: [
      {
        name: 'Borrow',
        type: 'column',
      ...
      {
        // ✅ Top totals directly on bar tips
        name: 'Total Labels',
        type: 'scatter',
        showInLegend: false,
