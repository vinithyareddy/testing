...
export class SsByLocationComponent implements AfterViewInit, OnDestroy {
  ...
  /**
   * Calculate the distance from the center of the globe to determine scaling
   */
  private getDistanceFromCenter(centroid: [number, number]): number {
    const projected = this.projection(centroid);
    if (!projected) return 0;
    
    const [x, y] = projected;
    const centerX = this.globeContainer.nativeElement.offsetWidth / 2;
    const centerY = this.globeContainer.nativeElement.offsetHeight / 2;
    
    const distance = Math.sqrt(Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2));
    const normalizedDistance = distance / this.currentRadius;
    
    return normalizedDistance;
  }

  /**
   * Calculate perspective scaling based on position on sphere
   */
  private getPerspectiveScale(centroid: [number, number]): number {
    ...
    return 0.6 + (dotProduct * 0.4);
  }

  /**
   * Create curved text path for labels that follow the globe surface
   */
  private createCurvedTextPath(text: string, centroid: [number, number], id: string): string {
    const projected = this.projection(centroid);
    if (!projected) return '';
    ...
    const curvature = (distanceFromCenter / this.currentRadius) * 20;
    
    const startX = x - textLength / 2;
    const endX = x + textLength / 2;
    const midY = y - curvature;
    
    return `M ${startX} ${y} Q ${x} ${midY} ${endX} ${y}`;
  }
