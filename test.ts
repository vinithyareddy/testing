...
export class AvgLaborCostRegionComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.http.get<any>('assets/data/world-globe-data.json').subscribe(data => {
      ...
      // Enhanced hover handling with proper country detection
      const handleHover = (event: MouseEvent) => {
        ...
        // Get all intersections with the globe
        const intersects = raycaster.intersectObjects(this.globe.children, true);
        
        if (intersects.length > 0) {
          const intersectedObject = intersects[0].object;
          
          // Try to find the country from the intersected object's userData
          let countryData = null;
          
          // Check if the object has country data
          if (intersectedObject.userData && intersectedObject.userData.feature) {
            const countryName = intersectedObject.userData.feature.properties.name;
            countryData = this.laborData.find(c => c.country === countryName);
          }
          
          // Fallback: find closest country based on intersection point
          if (!countryData) {
            ...
            for (const c of this.laborData) {
              ...
              if (dist < minDist && dist < 15) { // Only if reasonably close
                minDist = dist;
                closest = c;
              }
            }
          ...
          if (countryData) {
            ...
            tooltip.innerHTML = `<b>${countryData.country}</b><br>Region: ${countryData.region}<br>Avg Cost: $${countryData.cost}`;
