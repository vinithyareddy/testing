...
const HIGHLIGHT_COLOR = '#ff0000'; // Red color for highlighting
...
export class AvgLaborCostRegionComponent implements AfterViewInit, OnDestroy {
  ...
  private highlightedCountry: string | null = null; // Track highlighted country
  private highlightTimeout: any; // Timeout for hiding highlight
  ...
  private drawCountries() {
    ...
    this.svg.selectAll('.country')
      ...
      .attr('stroke', (d: any) => this.getCountryStroke(d))
      .attr('stroke-width', (d: any) => this.getCountryStrokeWidth(d))
      .attr('stroke-dasharray', (d: any) => this.getCountryStrokeDash(d))
  ...
  private getCountryStroke(d: any): string {
    const countryName = d.properties.name;
    if (this.highlightedCountry === countryName) {
      return HIGHLIGHT_COLOR;
    }
    return this.selectedView === 'By Country' ? STROKE_COLOR_COUNTRY : 'none';
  }

  private getCountryStrokeWidth(d: any): number {
    const countryName = d.properties.name;
    if (this.highlightedCountry === countryName) {
      return 3;
    }
  ...
  private getCountryStrokeDash(d: any): string {
    const countryName = d.properties.name;
    if (this.highlightedCountry === countryName) {
...
`;
  ...
  focusOnCountry(country: CountryCost) {
    ...
    this.highlightedCountry = country.country;
    ...
    // Show tooltip for the highlighted country
    this.showCountryTooltip(country);

    // Set timeout to remove highlight and tooltip after 3 seconds
  ...
  private showCountryTooltip(country: CountryCost) {
    const [x, y] = this.projection([country.lng, country.lat]) || [0, 0];
    
    let tooltipContent = '';
    if (this.selectedView === 'By Region') {
      const regionGroup = this.regionGroups.find(r => r.region === country.region);
      const regionTotal = regionGroup ? regionGroup.total : country.cost;
      tooltipContent = `
        <div class="tooltip-row tooltip-header">${country.region}</div>
        <div class="tooltip-row tooltip-body">
          <span>Average Cost</span>
          <span><b>$${regionTotal}</b></span>
        </div>
