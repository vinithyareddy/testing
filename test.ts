...
export class AvgLaborCostRegionComponent implements AfterViewInit {
  ...
  private countryColorScale = d3.scaleLinear<string>()
    .domain([0, 1]) // temporary placeholder
  ...
  ngAfterViewInit() {
    ...
    this.globe.globeMaterial(new THREE.MeshPhongMaterial({ color: new THREE.Color(DEFAULT_GLOBE_COLOR) }));
    ...
    // 🌍 Add a soft transparent sphere overlay to keep circular shape
    ...
    this.http.get<any>('assets/data/world-globe-data.json').subscribe(data => {
      this.laborData = data.countries.map((c: any) => ({
        country: c.name,
        region: c.region,
        cost: c.cost ?? Math.floor(Math.random() * 500 + 50), // fallback: random 50–550
        code: c.code
      }));
  ...
  private applyColors(mode: 'region' | 'country') {
    ...
    this.globe.polygonsData(this.countries.features)
      ...
      .polygonSideColor(() => 'rgba(0,0,0,0)') // hide jagged sides
