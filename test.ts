...
export class SsByLocationComponent implements AfterViewInit {
  ...
  private addCountryLabels() {
    ...
    // base number of labels depending on zoom
    const maxLabels = this.currentZoom < 120 ? 50 : this.currentZoom < 200 ? 30 : 15;

    // choose which list to display
    const sourceList = this.searchTerm ? this.filteredList : this.countriesList;

    sourceList
      .sort((a, b) => b.uniqueSkills - a.uniqueSkills) // highest skill first
      .slice(0, maxLabels) // only top N
      .forEach(country => {
        if (country.position) {
  ...
  private updateLabelVisibility() {
    ...
    const minDistance = 8; // avoid overlap
