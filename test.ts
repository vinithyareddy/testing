...
<div class="budget-card-box-lg" #cartboxchartsection [ngClass]="{ 'fullscreen': isFullscreen }">
  <div class="budget-box-chart-lg">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center flex-wrap">
      ...
      <!-- Left Section - Title -->
      <div class="widget-heading pointer mt-1 col-12 col-md-8 d-flex align-items-center justify-content-center justify-content-md-start mb-2 mb-md-0">
        <span class="title-with-icon d-flex align-items-center gap-2">
          {{ selectedView === 'By Region' ? 'Average Labor Cost by Region' : 'Average Labor Cost by Country' }}
          <ng-template [ngTemplateOutlet]="infotemp"></ng-template>
        </span>
      </div>

      <!-- Right Section - Controls -->
      <div class="col-12 col-md-4 d-flex justify-content-center justify-content-md-end align-items-center header-icons">
    ...
    <div class="globe-wrapper">
      ...
      <div class="zoom-container">
        <button (click)="zoomIn()" type="button" aria-label="Zoom In">+</button>
        <button (click)="zoomOut()" type="button" aria-label="Zoom Out">-</button>
      </div>
          ...
          <table class="legend-table">
            ...
            <tbody>
              ...
              <ng-container *ngIf="selectedView === 'By Region'">
                <ng-container *ngFor="let region of regionGroups; trackBy: trackByRegion">
                  
                  <!-- Region row -->
                  <tr (click)="expandRow(region)" class="pointer region-row" role="button" tabindex="0" 
                      (keydown.enter)="expandRow(region)" (keydown.space)="expandRow(region)">
                    <td>
                      <span class="cell-content">
                        <i *ngIf="!region.expanded" class="far fa-plus-circle expand-icon" aria-hidden="true"></i>
                        <i *ngIf="region.expanded" class="far fa-minus-circle expand-icon" aria-hidden="true"></i>
                        {{ region.region }}
                      </span>
                    </td>
                    <td class="cost-col">${{ region.total | number:'1.0-0' }}</td>
                  </tr>
                  ...
                  <tr *ngIf="region.expanded">
                    <td colspan="2" class="country-scroll-cell">
                      <div class="country-scroll">
                        <table class="w-100">
                          <tr *ngFor="let c of region.countries; trackBy: trackByCountry" 
                              class="country-row" 
                              (click)="focusOnCountry(c)"
                              role="button" 
                              tabindex="0"
                              (keydown.enter)="focusOnCountry(c)"
                              (keydown.space)="focusOnCountry(c)">
                            ...
                            <td class="cost-col">${{ c.cost | number:'1.0-0' }}</td>
                          </tr>
                        </table>
                      </div>
                    </td>
              ...
              <ng-container *ngIf="selectedView === 'By Country'">
                <tr *ngFor="let c of countryList; trackBy: trackByCountry" 
                    class="country-row" 
                    (click)="focusOnCountry(c)"
                    role="button"
                    tabindex="0"
    ...
    <!-- View More Section -->
    <div class="viewmore pointer mt-3 pt-3" role="button" tabindex="0">
      <span>View More&nbsp;&nbsp;</span>
