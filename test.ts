...
export class AvgLaborCostRegionComponent implements AfterViewInit, OnDestroy {
  ...
  // --- THREE.js Globe for Labels ---
  private initializeThreeGlobe() {
    const globeDiv = this.globeContainer.nativeElement;
    const width = globeDiv.offsetWidth;
    const height = globeDiv.offsetHeight;

    this.threeScene = new THREE.Scene();
    this.threeCamera = new THREE.PerspectiveCamera(40, width / height, 0.1, 1000);
    this.threeCamera.position.z = 400;

    this.threeRenderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
    this.threeRenderer.setSize(width, height);
    globeDiv.appendChild(this.threeRenderer.domElement);

    const geometry = new THREE.SphereGeometry(200, 64, 64);
    const material = new THREE.MeshBasicMaterial({
      color: 0x154361, // fallback if texture not loaded
      transparent: true,
      opacity: 0.2
    });
    this.globeMesh = new THREE.Mesh(geometry, material);
    this.threeScene.add(this.globeMesh);

    this.threeScene.add(this.labelGroup);
    this.animateThree();
  }

  private addLabel(lat: number, lon: number, text: string) {
    ...
    const radius = 205; // just above globe surface
    const phi = (90 - lat) * (Math.PI / 180);
  ...
  // -------------------
  // your D3/Dashboard logic unchanged
  // -------------------
  ...
  private setupInteractions() {
    ...
    this.globeContainer.nativeElement.addEventListener('wheel', () => {}, { passive: true });
  }
  ...
  private loadData() {
    this.http.get<any>('assets/json/world-globe-data.json').subscribe(data => {
      ...
      // --- add labels with Three.js ---
      this.laborData.forEach(c => {
        this.addLabel(c.lat, c.lng, c.country);
      });
    });
  }
      ...
      if (this.selectedView === 'By Region') {
        ...
        tooltipContent = `
          <div class="tooltip-row tooltip-header">
            <img src="assets/images/flags/${entry.code.toLowerCase()}.svg"
                 alt="${entry.country} flag" class="flag-icon" title="${entry.country}" />
